...
@Component({
  selector: 'app-update-color-device',
  templateUrl: './update-color-device.component.html',
  styleUrls: ['./update-color-device.component.scss']
})
export class UpdateColorDeviceComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.form = new FormGroup({
      ...
      color_code: this._formBuilder.control('', Validators.required)
    });
    ...
    this.subcribeTopic = this._activeRoute.params.subscribe((params: Params) => {
      ...
      this._service.getChangeTypes(id).subscribe((response: any) => {
        if (response.status === 204) {
          ...
          const value: ColorDevice = response.body;
          console.log(value);
          this.form.setValue({
              'id': value.id,
              'name': value.name,
